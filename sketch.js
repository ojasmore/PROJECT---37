...
function setup() {
  ...
  gameover = createSprite(camera.position.x,displayHeight/2-50);
  gameover.addImage("gameover",gameoverimg);
  gameover.scale = 0.6;
  gameover.visible = false;
  
  restart = createSprite(camera.position.x,displayHeight/2);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(camera.position.x + 700,400,40,10);
...
function spawnObstacles() {
  if(frameCount % 40 === 0) {
    var obstacle = createSprite(camera.position.x+700,485,10,40);
